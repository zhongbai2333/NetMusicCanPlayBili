 ...
 */
final class Nv12TextureSet implements VideoYuvTextureSet {
    ...
    Nv12TextureSet(Identifier yId, Identifier uvId, Identifier placeholderId, String labelPrefix) {
        this.yId = yId;
        this.uvId = uvId;
    ...
    public boolean upload(VideoBillboardPreview.DecodedFrame frame, int width, int height) {
        ...
        if (buffer != null) {
            return upload(buffer, frame.byteLength(), width, height);
        }
        return upload(frame.data(), width, height);
    }

    boolean upload(byte[] nv12, int width, int height) {
    ...
    boolean upload(ByteBuffer nv12, int byteLength, int width, int height) {
        int ySize = width * height;
        int uvWidth = Math.max(1, width / 2);
        int uvHeight = Math.max(1, height / 2);
        int uvSize = uvWidth * uvHeight * 2;

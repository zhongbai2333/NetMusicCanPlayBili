 ...
 *
 * <p>
 * 这是 YUV 渲染管线的第一阶段验证实现：native 解码器输出 Y + U + V 三平面后，先在 CPU
 * 转回 RGBA，继续复用现有 DynamicTexture 渲染链路。等 YUV 解码/队列/同步稳定后，再把这里替换为
 * GPU 三纹理 + fragment shader 路径。
 * </p>
 */
final class Yuv420pConverter {
    ...
    static byte[] toUploadRgba(Fmp4NativeVideoDecoder.DecodedFrame frame, int width, int height) {
    ...
    static byte[] yuv420pToRgba(byte[] yuv, int width, int height) {

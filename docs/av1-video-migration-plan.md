...
- 默认禁止 8K AV1 软件解码，也不应自动尝试 4K/8K 软件解码。
...
- 第一阶段使用 FFmpeg 内置 AV1 decoder，不引入 `libaom` 或 `dav1d`。性能不足时再单独评估 dav1d，并补充其 BSD 许可证与跨平台构建审计。

## 目标播放决策

### 默认模式 `auto`

默认策略不是“AV1 任意方式优先”，而是“AV1 硬解优先，软件 AV1 受性能预算约束”。
...
| 模式 | 行为 |
| --- | --- |
| `auto` | AV1 硬解优先；软解受安全上限和实时性能约束；默认值 |
| `prefer-av1` | AV1 硬解优先，并允许在安全上限内更积极地尝试软件 AV1 |
...
> 实施状态（当前工作区）：Java 请求、候选、首帧回退和 AV1 fMP4 基础路径已实现并通过干净全量测试；FFmpeg/JNI 已由 GitHub Actions 生成 `media-min-v36` 六平台产物，归档 SHA-256、路径结构和 LGPL 文件均已核验，六个平台目录已作为一个整体替换。后续仍需完成 Mod 构建、目标平台动态加载及真实 B站 AV1/设备矩阵验证。
...
- [ ] 六个平台动态加载与目标设备依赖审计通过；
- [ ] native 单元/烟雾测试能打开 AV1 软件 decoder 并解出冻结样本首帧。

完成标准：发布二进制不含 HEVC decoder，H.264 和 AV1 软件解码可验证。
...
完成标准：CPU 路径可稳定播放真实 AV1 DASH 样本，且 H.264 回归测试无变化。

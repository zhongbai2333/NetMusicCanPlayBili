...
## 当前实现状态（截至 2026-08-13）

本节是本文档中**当前实现状态的唯一真源**。其余章节中的数据结构、状态机和参数若未在本节标为已实现，均应理解为目标规格或历史实施记录，而不是现有代码保证。

### 2026-08-13 收口增量（优先于下方旧描述）

- 场景快照与 ACL 保存发生 revision 冲突时，服务端结果包现在直接携带同 revision 的完整权威文档；非冲突结果不携带文档。客户端继续保留本地草稿，“重新加载服务器版”固定使用该回包快照，不再依赖方块实体同步先后顺序。权威安装同时重置元素、ACL 与 autosave fingerprint，避免重载后产生伪 dirty 保存。
- 完整文档回包具有独立有界 codec，覆盖 schema、稳定身份、ACL、源绑定、hardRange 与全部元素；仍执行 64 KiB 场景预算，并为 ACL/文档元数据保留额外 8 KiB 上限。冲突文档非空和 revision 一致性已下沉为纯核心契约测试。
- 锁定元素修改被策略拒绝时不再误记 operationId 或返回 `APPLIED`，现在保持 revision 不变并返回 `REJECTED`。
- B站 API、WBI、直播解析和扫码登录 generate/poll/图片请求已统一持有可取消的根 `sendAsync` future；同步 worker 中断会取消 exact request，手持视频、歌曲解析和直播状态探测的 owner 也已传播取消。扫码 UI 关闭或换代会关闭 manager 并拒绝迟到纹理回调。
- OpenAL native delete batch 现在分别统计失败 batch、source 单项删除失败和 buffer 单项删除失败；Java 异常与 OpenAL error code 都会计数，后续句柄仍继续 best-effort 删除。
...
- terrain MID/FAR 已从分类线框升级为 4³/8³ 代表材质外露面：按出现频率、最高 Y 和输入顺序确定性选材，使用冻结的
  particle/still 纹理、光照和 tint。注册方块 tint layer 在客户端主线程预计算，后台不回调活动世界或模组颜色逻辑；
  核心范围内方块实体以提取后的 `BlockEntityRenderState` 进入 PIP；透明网格保留 sort state，并按 PIP 相机变化只更新
  quad index。UNKNOWN 仍为线框且不强载 chunk。
- macOS Apple M4 integrated-client terrain PIP 场景已取得总体/场景 `PASSED` 和有效环境：4³/8³ 材质单元、冻结 tint、
  蓝色染色玻璃透明上传、箱子方块实体提交与相机换位透明重排均真实命中，生成前后两张不同 SHA-256 截图，渲染失败为 0。
- 同一 Apple M4 integrated-client 已完成固定第三方矩阵：Accurate Textures 32× 资源成为权威纹理，BOP palm leaves tint、
  BOP blood fluid state、Colossal Chests render state 均进入不可变 frame 和真实 PIP；Iris 1.11.3 + Sodium 0.9.1
  明确启用 MakeUp Ultra Fast 9.5c。结构化指标中三类外部对象最大值均为 1，资源包/shaderpack active 恒为 1，
  材质上传 6、透明上传 2、透明重排 4、方块实体提交 69、失败 0；外部资产精确 SHA-512 校验且不随生产包分发。
- Apple M4 上的真实媒体最终消费者系统门槛已完成：`ncpb.real-media-lifecycle-100` 连续 100 轮同时建立真实
  Bilibili DASH H.264/native decoder、NV12/YUV texture/PBO 和 Bilibili 音频/Stereo OpenAL；每轮只有在 HTTP、
  decoder/关闭诊断、GPU、OpenAL 与全部自有内存回到捕获基线后才进入下一轮。报告总体/场景均 `PASSED`，
  单轮收敛观测最大 54 ms，Stereo OpenAL 创建、开始清理和完成清理均精确增加 100。
- dedicated server + 两个独立客户端 JVM 的 paired bench 已关闭多人门槛：正式 lease/在线玩家严格经历 2→1→0；
  计划断线客户端以相同 UUID 重连并重新取得 lease，幸存客户端 lease 不受影响；真实媒体变体中两端均加载
...
- Phase 8 的库边界已进一步迁移到独立的 [SceneEditor](https://github.com/zhongbai2333/SceneEditor) 项目：
  `scene-editor-core` 和 `scene-editor-minecraft` 由 JitPack 发布 `1.0.1-beta.2` 的 main/sources/Javadoc artifact，
...
- 当前隔离完整回归为 **229 suites / 874 tests / 0 failures / 0 errors / 0 skipped**；生产 JAR、embedded native、法律材料、冲突副本、Scene Editor API/JiJ 和双宿主静态去重门槛通过，`git diff --check` 无 whitespace error。
...
中控台屏幕已有独立 `IDLE / BUFFERING / ERROR / ACTIVE` 运行时状态：源停止或纯音频为 IDLE，解析/等待首帧为 BUFFERING，解析或解码终态失败稳定保留 ERROR，只有真实上传视频帧才进入 ACTIVE。消费者有效时屏幕始终提交画面，不再因无帧消失；IDLE/BUFFERING/ERROR 使用三张中控台专属 320×180 像素风静态纹理，ACTIVE 继续显示真实共享视频帧。屏幕世界几何始终使用元素持久化的 `aspect`，不再随视频或占位纹理比例跳变。
...
OpenAL source/buffer 的关闭也已按 native batch 分配纯标量 operationId：实例 cleanup 转移句柄所有权后立即释放 staging buffer，native batch 进入单飞串行删除队列；无可用 context 时标记 deferred，客户端 tick 每 500 ms 节流重试，后续 init 获得 context 时也会 drain。只有 best-effort source/buffer 删除循环返回后才记录收敛，默认 500 ms soft timeout、3 秒 hard timeout，并统计 pending/deferred batch、最老年龄、迟到收敛、失败 batch 以及 source/buffer 单项删除失败。relay 与 Dolby/Stereo handler 的 terminal cleanup 已原子幂等，registry 替换/注销会先从 handler detach 再 cleanup，旧 handler terminal cleanup 后清空 relay 引用。该完成点仍只表示 OpenAL 删除调用循环已经返回，不证明驱动物理内存即时回落。
...
- 中控台 IDLE/BUFFERING/ERROR 专属静态美术、ACTIVE 真实共享帧映射，以及三张资源的尺寸/不透明性/差异自动校验；
...
- terrain：hardRange、UNKNOWN、chunk load/unload tombstone、NEAR 原版全材质、MID/FAR 代表材质简化、方块实体
  render state、注册方块 tint 冻结、quad 级透明索引重排和 GPU 降级释放已接通；vanilla 与固定第三方资源包、
  自定义 tint/流体/方块实体、Iris/shaderpack PIP 实机矩阵均已通过。

### 已完成的最终系统门槛

1. 真实媒体最后消费者 100 轮：真实 Bilibili H.264 视频/native decoder/NV12/PBO 与音频/OpenAL 同时有负载，
   每轮 HTTP、decoder、GPU、OpenAL 和自有内存均先回到基线，最终精确 100 轮；
2. 多客户端独立消费者：两个独立客户端 JVM 与 dedicated server 的 lease 2→1→0、幸存者不受影响、最终清理通过；
3. 断线重连：同 UUID 重连和正式 lease 重获通过，另一客户端的 lease 在对端断线期间保持；
4. 真实媒体 Iris/shaderpack：两个独立客户端均实际加载 Bilibili 视频/音频、VideoToolbox、NV12/PBO、OpenAL，
   并由 Iris API 确认 MakeUp shaderpack 正在使用，随后分别回到各自基线；
5. 跨维度：此前独立 integrated-client 场景已经覆盖 respawn/clone packet、ClientLevel unload 与 exact media cleanup。
...
- hardRange 已用于服务端消费资格、客户端即时门控、边界增益、250 ms 视频/音频/字幕退出与重入包络和中控台消费者注销；异步 resolve 已不能在最后消费者退出后复活实例；真实媒体 100 轮已经证明当前 Apple M4 组合中 API HTTP、decoder、GPU 和 OpenAL 均在下一轮开始前回到捕获基线；
- terrain 已覆盖权威 hardRange、UNKNOWN/NEAR/MID/FAR、chunk tombstone、材质简化、方块实体 render state、冻结 tint 与透明重排，并有 vanilla 及固定第三方资源包/模组/Iris shaderpack 的 PIP 实机证据；该 Apple GPU 单组合**不能外推为所有模组、资源包或硬件**；
- 中控台直播视频、直播音频 relay 及标题/房间/状态元数据字幕已接通；唱片机 AI CC 已具备共享会话所有权、精确取消、权威时间线和安全降级，但这不替代真实公网字幕请求的设备矩阵验收；
...
在当前固定 Apple M4/VideoToolbox/Iris/MakeUp 组合内，中控台已经满足真实媒体最终资源生命周期、物理多客户端、
断线重连和 shaderpack 系统验收。对外仍应明确其跨操作系统、GPU、驱动及更广模组/资源包矩阵尚未完成，不能把
单一组合结果表述为全平台兼容认证。
...
以上是 Phase 0 冻结的历史基线，不代表当前相机实现；当前 orbit、pan、dolly、fly、正交/透视和统一矩阵状态见“当前实现状态”与 Phase 1 记录。该历史清单中的 pivot、skew、非均匀 scale、本地/世界 Gizmo 和统一命令栈现均已由 schema v6/Phase 53 完成。
...
| 地形预览逻辑范围 | 等于硬范围 |
| 当前地形细节 | NEAR 原版全材质；MID 4³、FAR 8³ 代表材质外露面；核心方块实体 render state；冻结 tint；透明 quad 索引重排；未加载区域 UNKNOWN |
...
截至 schema v6，Phase 1 的完整对象变换目标也已落地：屏幕/字幕使用 position、yaw/pitch/roll、三轴非均匀 scale、三轴 pivot 和二维 skew；Gizmo 可切本地/世界空间，世界旋转和世界缩放分别使用四元数组合与平面基分解；PIP、世界 renderer、灵魂漫游和拾取共享同一变换顺序。命令栈已经覆盖连续拖动、数值/内容字段、集合操作、预设、中控台属性和漫游返回。
...
当前 P3 已落地可运行编辑器纵切：控制台右键直接进入三栏场景编辑器；左侧层级可新增屏幕、字幕和音频三类本地元素，并支持选择、复制和删除；中央视口在控制台宿主模式渲染实际中控台方块模型而非玩家模型；元素支持完整变换输入、本地/世界 Gizmo、仿射矩形拾取与统一撤销/重做。无元素选中时右侧显示中控台自身的名称、绑定源、revision 和硬范围检查器；选中元素时则在完整变换与类型内容两页之间切换。旧三点数值面板入口已移除。
...
- **AI 字幕模式**：只选择 B站标记为 AI 的 CC 轨；同一 source/session 的中控台与歌词投影仪共享任务，按统一媒体时钟选行，并在不可用/失败时安全降级；
...
- 直播机的视频总线和 source-position 音频 relay 已接入同一套元素运行时；直播标题、房间信息和状态元数据字幕使用 source/session 级共享缓存。AI CC 当前绑定 B站点播唱片机源，不伪造直播 AI 字幕来源。
...
- 地形始终使用纯核心/不可变 section 快照和独立 PIP；原版模型/流体、代表材质远景、注册方块 tint 与动态方块实体
  render state 通过可熔断适配层接入，任何模组回调异常只降级对应单元或实体，不得中断整个 section 或客户端。
- 主线程只负责在单元数、section 数和墙钟时间三重预算内读取已加载世界并生成不可变快照；后台只消费快照进行 LOD/meshing。

NEAR 路径沿用经过压力验证的原版全材质实现。`TerrainPreviewManager` 会按覆盖游标渐进捕获 hardRange 内已加载 section；NEAR 快照保存中央 16³ 内非空气、`RenderShape.MODEL` 方块及外扩两格后的 20³ 不可变 `BlockState` 和光照邻域。模型编译使用世界 `BlockPos` 查询邻接状态和计算随机种子，顶点仍输出为 section 局部坐标；`ModelBlockRenderer` 负责按原版规则剔除隐藏面和计算 AO，包括跨 section 边界的相邻方块。完全处于核心范围外的 MID/FAR section 分别聚合为 4³/8³ 单元，以出现频率、最高 Y、输入顺序确定代表 `BlockState`，使用 particle/still 纹理生成外露 cuboid 面；LOD 降级会移除对应 NEAR GPU resident。与球形 NEAR 核心相交的边界 section 保持细节路径，避免简化网格与近景重叠。
...
PIP/compiler 只消费不可变快照，不持有 `ClientLevel`、chunk 或活动方块实体。20³ 捕获会预先查询固定 3×3 邻接 chunk 的加载状态（9 次），并预计算 20×20 个 X/Z 列的硬范围和可读性，不再为 8,000 个单元重复进行水平边界及 chunk 索引判断；状态和光照共享同一个可变坐标，光照数组只在最终快照不可变边界复制一次。核心范围内最多 128 个方块实体在客户端 tick 主线程刷新为提取后的 `BlockEntityRenderState`，PIP 只提交这些状态。无 section/pending/动态实体变化时复用已发布的 `TerrainPreviewFrame`；PIP 对同一帧也复用覆盖判定。方块更新会重新捕获自身及边界相邻 section，旧 GPU 网格保留到新版本上传后再原位替换。generation、硬范围变化、资源包模型集合变化和 PIP renderer 关闭才释放不再有效的 GPU section；失败快照保留旧网格且同一版本不重复刷日志。低频覆盖日志额外报告 `capturePending` 和 `cpuBuffered`，用于确认 pending 持续下降且待编译快照不超过背压上限。

当前持久网格仍有明确画质边界：方块模型捕获 `RenderShape.MODEL`，非空 `FluidState` 通过原版 `FluidRenderer` 接入；MID/FAR 是代表 particle/still 材质的 cuboid 外露面，不等于完整远景模型几何。20×20×20 邻域 `BlockState`、流体、方块光与天空光快照已经启用；每个光照单元以 4 bit 方块光和 4 bit 天空光压缩到一个 byte，`LevelRenderer` 在模型编译时据此生成原版 packed light，发光方块的自发光规则仍由原版实现补正。`ModelBlockRenderer` 已启用原版 AO 和邻接面剔除。客户端 tick 主线程会冻结每个可见方块已注册的 tint layer（含群系混色结果），后台 `FrozenTerrainBlockColors` 只读取纯整数。透明层在首次编译时保存 `SortState` 和辅助索引空间，PIP 相机变化时只重排可见 section 的 quad index；顶点、世界快照和非透明层不重建。自定义流体颜色是否能通过对应方块 tint 覆盖仍取决于模组注册方式，必须在真实模组矩阵中验证。
...
- `TerrainPreviewManager`/`TerrainPreviewFrame`：当前管理全范围 section 排序、可见 `BlockState`/tint/light 捕获、代表材质聚合、核心方块实体 render state 提取、dirty 重捕获、256 MiB LRU 和不可变帧发布；
- `TerrainNeighborhoodIndex` 和 20³ 邻域快照：为跨 section 模型连接、随机种子、真实方块/天空光、原版隐藏面剔除及 AO 提供不可变世界上下文；
- `TerrainPreviewSectionCompiler`：仅消费冻结的模型集、颜色表和不可变快照，按真实 `ChunkSectionLayer` 后台编译当前资源包方块模型与 MID/FAR 代表材质外露面，并为透明层保留 quad sort state；
- `TerrainPreviewGpuCache`：调度单飞 CPU 编译、每帧最多上传一个完成 section，在共享分层 `UberGpuBuffer` 中持久保存 TLSF allocation，以每层一次 `drawMultipleIndexed` 批量提交所有可见 section；透明层随 PIP 相机只更新索引 heap，并在 generation/资源重载/关闭时释放。
...
区块 load/unload 已通过 tombstone 和自然重捕获收敛；方块实体 render state、注册方块 tint 冻结和 quad 级透明索引重排已接入。vanilla integrated-client 已覆盖真实 PIP、连续相机换位、透明上传/重排、冻结 tint 与方块实体提交；固定第三方矩阵进一步覆盖 Accurate Textures、BOP tint/流体、Colossal Chests render state 以及 Iris/MakeUp shaderpack。共享 `UberGpuBuffer`、TLSF allocation 与 `drawMultipleIndexed` 已在实测证明独立 section buffer/逐次提交构成瓶颈后接入。
- 加入正视、侧视、顶视、透视和第一人称预览。

### Phase 5：视觉元素与占位状态

状态：**已完成当前设计纵切，待系统矩阵验收**。唱片机/直播屏幕、字幕世界渲染、多屏共享帧、中控台 `IDLE / BUFFERING / ERROR / ACTIVE` 状态机、IDLE/BUFFERING/ERROR 专属静态美术、ACTIVE 真实共享帧、元素 aspect、字幕主色/译色/背景/对齐/宽度/换行、直播标题/房间/状态元数据字幕，以及点播 AI CC 的共享所有权、统一时间线和安全降级均已完成。

- 接入屏幕和字幕世界渲染。
- 已为四态运行时接入中控台专属空闲、缓冲和错误图片；活动态显示真实视频帧。
- 复用全息占位层的双面纹理提交模式，但保持独立纹理与语义。
- 多屏共享同一视频帧和纹理。

### Phase 6：音源、播放与硬范围

状态：**当前设计纵切与固定设备系统门槛已完成**。唱片机/直播统一源适配、共享视频和 PCM、多空间 relay、音量、声道、独立最大听距、autoMixJoc、每玩家消费租约、客户端 hardRange 即时门控、4 格距离淡变、2 格重入迟滞、250 ms 视频/音频/字幕退出与重入包络、中控台消费者截止清理、异步解析实时准入、迟到结果防复活、轻量资源诊断、视频五阶段关闭诊断、OpenAL native batch/deferred drain，以及 HTTP Range/分片/CDN race/HTTP-FLV/B站 API/登录二维码的请求级取消与关闭诊断已完成；确定性 GPU 上传/释放和真实 Bilibili 视频/音频 100 轮最终消费者资源基线均已通过。跨设备兼容范围另行扩展。
...
`[1.0.1-beta.2,2.0.0)`。客户端 integrated-client 和专用服务器均已实际加载；Minecraft adapter 必须使用
